        ...
        try:
            if request.form['Ask'] == 'Ask':
                ...
                for row in data:
                    ...
                    ved = "update trained_set set percen='" + str(v) + "' where query='" + str(que) + "'"
                    mm.insert_query(ved)

            qry = "SELECT response from trained_set where percen='1'"
            xa = mm.select_direct_query(qry)
            xa = xa[0][0]
            qrky = "SELECT graphical from trained_set where percen='1'"
            bv = mm.select_direct_query(qrky)
            bv = bv[0][0]
